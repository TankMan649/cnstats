...
def set_proxy(proxy_url):
    """
    Set proxy for all API requests.
...
def easyquery(m='QueryData', dbcode='hgyd', rowcode='zb', colcode='sj', wds=[], dfwds=[], id=None, wdcode=None):
    """
    Query the stats.gov.cn API with optional proxy support.

    To use a proxy, first call set_proxy('http://proxy:port')
    """
    ...
    proxies = _proxy_config if _proxy_config else None

    r=requests.post(url, data=obj, headers=_header, verify=False, proxies=proxies, timeout=30)
    return r.json()
