#!/usr/bin/env python3
"""
Master Variables List Builder for stats.gov.cn (Proxy-Enabled)
...
PROXY SUPPORT:
This version supports using a proxy server to bypass IP-based geo-blocking.

Usage:
    # With HTTP proxy
    python build_master_variables_list_with_proxy.py --proxy http://proxy.example.com:8080

    # With SOCKS proxy
    python build_master_variables_list_with_proxy.py --proxy socks5://proxy.example.com:1080

    # With authentication
    python build_master_variables_list_with_proxy.py --proxy http://user:pass@proxy.example.com:8080
...
from cnstats.common_proxy import easyquery, set_proxy, clear_proxy
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Build master variables list for stats.gov.cn with proxy support'
    )
    parser.add_argument(
        '--proxy',
    ...
    parser.add_argument(
        '--output',
        help='Output filename for master variables list',
        default='master_variables_list_api.json'
    )

    args = parser.parse_args()

    print("=" * 80)
    print("Building Master Variables List for stats.gov.cn (API Discovery)")
    print("=" * 80)
    print()
    ...
    stats['proxy_used'] = args.proxy if args.proxy else 'None'
